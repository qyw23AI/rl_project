...
- 容器启动逻辑（VNC/VirtualGL）： [sh/docker_entrypoint.sh](../sh/docker_entrypoint.sh)
- Python 依赖清单： [requirements.txt](requirements.txt)
- 构建上下文过滤： [.dockerignore](.dockerignore)
- 本地构建+测试+推送： [sh/build_and_push.sh](../sh/build_and_push.sh)
- 服务器一键拉取/构建/运行： [sh/run_remote.sh](../sh/run_remote.sh)
...
- 含义：容器启动后执行 [sh/docker_entrypoint.sh](../sh/docker_entrypoint.sh)。

---

## 2. 学会读启动脚本 sh/docker_entrypoint.sh

### 2.1 严格模式

- 位置： [sh/docker_entrypoint.sh](../sh/docker_entrypoint.sh#L2)
- 含义：`set -euo pipefail` 让脚本尽早失败，减少“悄悄出错”。

### 2.2 初始化 VirtualGL

- 位置： [sh/docker_entrypoint.sh](../sh/docker_entrypoint.sh#L17-L27)
- 作用：首次运行配置 `vglserver_config`，并对非交互返回码做容错。

### 2.3 启动 VNC（24-bit）

- 位置： [sh/docker_entrypoint.sh](../sh/docker_entrypoint.sh#L37)
- 重点：`-depth 24` 可减少 OpenGL 黑屏/花屏问题。

### 2.4 保持容器前台运行

- 位置： [sh/docker_entrypoint.sh](../sh/docker_entrypoint.sh#L50)
...
- 对照文件： [sh/build_and_push.sh](../sh/build_and_push.sh#L11)
...
- 期望输出：`True`
- 对照文件： [sh/build_and_push.sh](../sh/build_and_push.sh#L15)

### 检查点 C：再推送镜像

- 对照流程：
   - 打标签 [sh/build_and_push.sh](../sh/build_and_push.sh#L17-L18)
   - 推送 [sh/build_and_push.sh](../sh/build_and_push.sh#L21)

---

## 5. 服务器一键部署脚本怎么读（最新版）

看 [sh/run_remote.sh](../sh/run_remote.sh)：
...
2. 若仍慢，使用本机代理转发：
   - 本机执行：`cd sh && SSH_TARGET=<user>@<server_ip> LOCAL_PROXY_PORT=7890 ./proxy_forward.sh`
   - 服务器执行：
...
- 结论：Docker 未安装或会话未生效
- 处理：执行 [sh/install_docker_gpu.sh](../sh/install_docker_gpu.sh)，然后重新登录 SSH

### 报错 B：`failed to resolve source metadata for docker.io/docker/dockerfile:1 ... i/o timeout`

- 结论：到 Docker Hub 的网络超时（常见于慢网/未代理）
- 处理：
   1. 跑 [sh/enable_mirror_acceleration.sh](../sh/enable_mirror_acceleration.sh)
   2. 开启 [sh/proxy_forward.sh](../sh/proxy_forward.sh)
  3. 确认服务器已导出 `HTTP_PROXY/HTTPS_PROXY`
   4. 重新执行 [sh/run_remote.sh](../sh/run_remote.sh)
...
- [sh/run_remote.sh](../sh/run_remote.sh)：远端一键同步 + 构建 + 运行
- [sh/install_docker_gpu.sh](../sh/install_docker_gpu.sh)：Docker + NVIDIA runtime 安装
- [sh/enable_mirror_acceleration.sh](../sh/enable_mirror_acceleration.sh)：国内网络加速与镜像配置
- [sh/proxy_forward.sh](../sh/proxy_forward.sh)：本机代理转发到远端
- [sh/build_and_push.sh](../sh/build_and_push.sh)：本地构建并推送镜像
