...
if [[ -d "${REPO_DIR}/rsl_rl/.git" || -f "${REPO_DIR}/rsl_rl/.git" ]]; then
  echo "[sync] Ensuring rsl_rl is on tag v1.0.2..."
  git -C "${REPO_DIR}/rsl_rl" fetch --tags --force || true
  git -C "${REPO_DIR}/rsl_rl" checkout v1.0.2 || true
fi
