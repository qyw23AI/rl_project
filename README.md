# 一、RL Project 环境配置相关DOCKER (Isaac Gym + MuJoCo + VirtualGL + TurboVNC)

本仓库已包含你要求的依赖与流程：

新手逐步学习与验收文档： [understand(docker).md](./md/understand%28docker%29.md)

脚本统一使用方式（推荐）：

- 先进入脚本目录：`cd sh`
- 然后执行脚本：`bash ./run_remote.sh`（其余脚本同理）
...
- [sh/run_remote.sh](sh/run_remote.sh) 已包含：
...
推荐运行命令（与 [sh/run_remote.sh](sh/run_remote.sh) 保持一致）：
...
- 先 `cd sh`，再执行：`sudo bash ./install_docker_gpu.sh`
...
如果你在国内网络环境下拉包较慢，可以先在远端设置代理，再进入 `sh` 目录执行 `run_remote.sh`：
...
一键转发脚本见 [sh/proxy_forward.sh](sh/proxy_forward.sh)：

- 在你电脑本机运行它，不是在服务器上运行。
- 它会把你本机的代理端口通过 SSH 反向转发到服务器上的 `127.0.0.1:17890`。
- 服务器上再导出：`HTTP_PROXY=http://127.0.0.1:17890`、`HTTPS_PROXY=http://127.0.0.1:17890`。
- 例子（先 `cd sh`）：`SSH_TARGET=ubuntu@10.60.20.189 LOCAL_PROXY_PORT=7890 ./proxy_forward.sh`
- 然后在服务器里先 `export HTTP_PROXY=http://127.0.0.1:17890`，再执行 `./run_remote.sh`

## 6) 服务器网络加速（推荐先执行）

如果服务器下载很慢，先执行一键加速脚本 [sh/enable_mirror_acceleration.sh](sh/enable_mirror_acceleration.sh)：

- 先 `cd sh`，再执行：`sudo bash ./enable_mirror_acceleration.sh`
...
1. 本机开代理并运行 [sh/proxy_forward.sh](sh/proxy_forward.sh) (已取消，不好用)
2. 服务器执行 `export HTTP_PROXY=http://127.0.0.1:17890` 与 `export HTTPS_PROXY=http://127.0.0.1:17890`
3. 服务器先 `cd sh`，再执行 `sudo bash ./enable_mirror_acceleration.sh`
4. 服务器在 `sh` 目录执行 `./run_remote.sh`
...
- `cd sh && sudo bash ./install_docker_gpu.sh`

2. （可选）本机打开代理转发到服务器：

- `cd sh && SSH_TARGET=<user>@<server_ip> LOCAL_PROXY_PORT=7890 ./proxy_forward.sh`
...
- `cd sh && sudo bash ./enable_mirror_acceleration.sh`
...
脚本入口： [sh/run_remote.sh](sh/run_remote.sh)

当前脚本支持三种常见模式：

1. 首次部署（构建 + 新建并启动容器）

- `bash ./run_remote.sh`（先进入 `sh` 目录）

2. 只启动已有容器（不重建镜像）

- `SKIP_BUILD=1 bash ./run_remote.sh`（先进入 `sh` 目录）
- 适用场景：你已经有可用镜像，只想快速把容器拉起来。

3. 强制重建容器（可选是否跳过构建）

- 重建镜像并重建容器：
  - `RECREATE_CONTAINER=1 bash ./run_remote.sh`（先进入 `sh` 目录）
- 不构建镜像，仅删除旧容器并基于现有镜像重建：
  - `SKIP_BUILD=1 RECREATE_CONTAINER=1 bash ./run_remote.sh`（先进入 `sh` 目录）
...
- [sh/install_docker_gpu.sh](sh/install_docker_gpu.sh)
  - 安装 Docker Engine、Compose 插件、NVIDIA Container Toolkit
  - 配置 Docker GPU runtime
- [sh/enable_mirror_acceleration.sh](sh/enable_mirror_acceleration.sh)
  - 配置 APT / Docker / pip / conda / git 慢网优化
- [sh/proxy_forward.sh](sh/proxy_forward.sh)
  - 在本机创建 SSH 反向转发，把本机代理暴露给服务器
- [sh/run_remote.sh](sh/run_remote.sh)
...
- [sh/build_and_push.sh](sh/build_and_push.sh)
...
- 处理：执行 [sh/install_docker_gpu.sh](sh/install_docker_gpu.sh)

2. `failed to resolve source metadata for docker.io/docker/dockerfile:1: ... i/o timeout`

- 原因：到 Docker Hub 网络超时（BuildKit 前端镜像拉取失败）
- 处理：
  - 先跑 [sh/enable_mirror_acceleration.sh](sh/enable_mirror_acceleration.sh)
  - 使用 [sh/proxy_forward.sh](sh/proxy_forward.sh) + `HTTP_PROXY/HTTPS_PROXY`
  - 当前 [sh/run_remote.sh](sh/run_remote.sh) 已默认回退到 legacy builder
...
- 处理：
  - 保持代理开启
  - 进入 `sh` 目录后重新执行 `./run_remote.sh`（已启用子模块并行拉取）
...
- 原因：容器用户与宿主目录属主不一致
- 处理：
  - 使用 `-u $(id -u):$(id -g)`（已在 [sh/run_remote.sh](sh/run_remote.sh) 默认启用）
...
- 在 `sh` 目录执行 `./run_remote.sh` 构建并启动成功
- 进入容器后：
